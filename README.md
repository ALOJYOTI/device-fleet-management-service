...
The project demonstrates strong Low-Level Design, concurrency control, API design, state management, and production-grade backend engineering.
...
## grpc API definition (device.proto)
...
service DeviceFleetService {
  rpc RegisterDevice(RegisterDeviceRequest)
      returns (RegisterDeviceResponse);

  rpc SetDeviceStatus(SetDeviceStatusRequest)
      returns (SetDeviceStatusResponse);

  rpc GetDeviceInfo(GetDeviceInfoRequest)
